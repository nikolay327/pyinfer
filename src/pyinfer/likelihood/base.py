...
class Sequential(LikelihoodBase):
    ...
    def __call__(self, *args, reduction="sum"):
        assert len(args) == len(self.likelihoods), "Number of arguments must match number of likelihoods"
        assert len(self.likelihoods) > 0, "No likelihoods to evaluate"

        log_likelihoods = [
            likelihood(arg) for likelihood, arg in zip(self.likelihoods, args)
        ]
